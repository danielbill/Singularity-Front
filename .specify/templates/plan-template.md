...
基于 SF-NEWS 项目章程 ([`.specify/memory/constitution.md`](../.specify/memory/constitution.md)) 的合规性验证：

- [ ] **I. 数据源可靠性**: 是否引入新的新闻数据源？如有，是否经过验证？是否使用了禁止来源？
- [ ] **II. 模块独立性**: 新功能是否清晰归属于某个模块？是否保持了模块边界？是否通过明确定义的接口通信？
- [ ] **III. 测试优先**: 是否包含测试计划？测试文件是否放在 `tests/` 目录？预计覆盖率是否≥80%？
- [ ] **IV. 中文优先**: 用户界面和文档是否使用简体中文？
- [ ] **V. 简单性原则**: 是否遵循 YAGNI 原则？是否引入了不必要的复杂性？

### 技术约束检查

- [ ] 技术栈是否符合项目规范（Python 3.11+、FastAPI、原生前端等）？
- [ ] 如涉及爬虫功能，抓取间隔是否≥15分钟？
- [ ] 如涉及数据存储，是否使用 SQLite + 文件系统模式？
- [ ] 关键词配置是否使用数组格式 `["关键词"]`？
